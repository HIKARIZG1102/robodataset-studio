...
from robodataset_studio.frontend.api_client import ApiClient
from robodataset_studio.core.runtime_env import apply_ros_environment, default_ros_setup
...
class BackendProcess:
    ...
    def start(self) -> None:
        ...
        env["ROBODATASET_BACKEND_HOST"] = self.host
        env["ROBODATASET_BACKEND_PORT"] = str(self.port)
        env["ROBODATASET_ROOT"] = str(self.root_dir)
        env.setdefault("ROS_SETUP", default_ros_setup())
        apply_ros_environment(env)
        log_dir = Path(tempfile.gettempdir())
        self.log_path = log_dir / f"robodataset_studio_backend_{self.port}.log"
        log_file = self.log_path.open("w", encoding="utf-8")
        python = self._python_executable()
        self.process = subprocess.Popen(
            [python, "-m", "robodataset_studio.backend.main"],
    ...
    def _health_compatible(self, data: dict) -> bool:
        if data.get("status") != "ok" or data.get("service") != "robodataset-studio":
            return False
