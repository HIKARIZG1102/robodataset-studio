...
from robodataset_studio.core.runtime_env import select_rmw
from robodataset_studio.ros.image_conversion import is_image_message_type
...
class InspectorDock(QWidget):
    ...
    def __init__(self, api: ApiClient) -> None:
        ...
        self.auto_contrast.setToolTip("Auto contrast preview")
        self.auto_contrast.setChecked(True)
        self.auto_contrast.setMinimumWidth(1)
        self.auto_contrast.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Fixed)
    ...
    def _topic_page(self) -> QWidget:
        ...
        node_button_row.addWidget(start_node)
        node_button_row.addWidget(stop_node)
        node_button_row.addStretch(1)

        topic_row = QHBoxLayout()
        topic_label = QLabel("Topic")
        topic_label.setToolTip("Generic topic")
        topic_label.setMinimumWidth(1)
        topic_label.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Fixed)
        ...
        layout.addLayout(node_button_row)
    ...
    def _image_page(self) -> QWidget:
        ...
        layout = QVBoxLayout(page)
        topic_row = QHBoxLayout()
        image_topic_label = QLabel("Image")
        image_topic_label.setToolTip("Image monitor topic")
        image_topic_label.setMinimumWidth(1)
        image_topic_label.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Fixed)
        topic_row.addWidget(image_topic_label)
        topic_row.addWidget(self.image_topic, 1)
        self.image_topic.currentTextChanged.connect(self.update_image_topic_type)
        project_row = QHBoxLayout()
        project_monitor = QPushButton("Sync from project")
        ...
        start = QPushButton("Start monitor")
        stop = QPushButton("Stop monitor")
        ...
        stats.setToolTip("Show frame stats")
        ...
        for button in [project_monitor, start, stop, pause, stats]:
            button.setSizePolicy(QSizePolicy.Minimum, QSizePolicy.Fixed)
        project_row.addWidget(project_monitor)
        ...
        secondary_controls.addWidget(pause)
        secondary_controls.addWidget(stats)
        secondary_controls.addWidget(self.auto_contrast)
        secondary_controls.addWidget(self.playback_fps)
    ...
    def start_image_preview(self) -> None:
        ...
        command = [
            sys.executable,
            "-m",
            "robodataset_studio.ros.image_preview_cli",
            "--topic",
            topic,
            "--message-type",
