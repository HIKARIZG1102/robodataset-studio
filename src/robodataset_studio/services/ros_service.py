...
from robodataset_studio.core.runtime_env import apply_ros_environment, available_rmw_implementations, default_ros_setup, select_rmw
...
class RosService:
    ...
    def image_snapshot(self, topic: str, *, timeout: float = 5.0) -> dict[str, Any]:
        ...
        try:
            ...
            node = rclpy.create_node(f"robodataset_image_snapshot_{uuid4().hex[:8]}", context=context)
    ...
    def _topic_info_rclpy(self, topic: str, *, timeout: float = 3.0) -> dict[str, Any]:
        ...
        try:
            rclpy.init(context=context)
            node = rclpy.create_node(f"robodataset_topic_info_{uuid4().hex[:8]}", context=context)
            executor = SingleThreadedExecutor(context=context)
            executor.add_node(node)
            deadline = time.time() + timeout
            msg_types: list[str] = []
    ...
    def _echo_once_rclpy(self, topic: str, *, timeout: float = 5.0) -> dict[str, Any]:
        ...
        try:
            rclpy.init(context=context)
            node = rclpy.create_node(f"robodataset_topic_echo_{uuid4().hex[:8]}", context=context)
            executor = SingleThreadedExecutor(context=context)
            executor.add_node(node)
    ...
    def _topic_hz_rclpy(self, topic: str, *, timeout: float = 5.0, window: int = 10) -> dict[str, Any]:
        ...
        try:
            rclpy.init(context=context)
            node = rclpy.create_node(f"robodataset_topic_hz_{uuid4().hex[:8]}", context=context)
            executor = SingleThreadedExecutor(context=context)
            executor.add_node(node)
            msg_type = self._discover_topic_type(node, executor, topic, timeout=min(timeout, 2.0))
            if not msg_type:
